...
var _element = null;
// 验证码元素ID，生成的dom将会挂载在该元素上  cxp 2023-01-30
var _captchaId = '';
// 验证码的宽高
var _captchaWidth = 270;
var _captchaHeight = 260;
var _appId = "";
...
function show() {
  ...
  // 显示验证码弹出层
  document.getElementById("simCaptcha-layer").className = "simCaptcha-show";
  // 设置验证码图片宽高 cxp 2023-01-30
  document.getElementById("simCaptcha-img").style.width = _captchaWidth + 'px';
  document.getElementById("simCaptcha-img").style.height = _captchaHeight + 'px';
}
...
function init() {
  var htmlLayer =
    '<div id="simCaptcha-mask" class="simCaptcha-hidden"></div>\
                ...
                <button style="widht: 0;height: 0; opacity: 0; position: fixed; left: -99999px" id="simCaptcha-btn-close">&#xe60a;</button>\
								<button id="simCaptcha-btn-refresh">&#xe675;</button>\
								<button id="simCaptcha-btn-confirm">确认</button>\
							</div>\
						</div>';
  // 容器上 插入验证码弹, 初始隐藏 cxp 2023-01-30
  // https://developer.mozilla.org/en-US/docs/Web/API/Element/insertAdjacentHTML
  // https://segmentfault.com/q/1010000003697751
  document.querySelector(`#${_captchaId}`).insertAdjacentHTML("beforeend", htmlLayer);
...
function SimCaptcha(options) {
  ...
  _captchaId = this.options.captchaId;
  _captchaWidth = this.options.width || 270;
  _captchaHeight = this.options.height || 260;
